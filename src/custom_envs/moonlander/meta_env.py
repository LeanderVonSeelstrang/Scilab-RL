...
class MetaEnv(gym.Env):
    def __init__(self):
        self.ROOT_DIR = "."
        config_path_dodge_asteroids = os.path.join(get_original_cwd(),
                                                   "custom_envs/moonlander/standard_config.yaml")
        config_path_collect_asteroids = os.path.join(get_original_cwd(),
    ...
    def step(self, action: int):
        ...
        match action:
            case 0:
                ...
                if self.current_task == 0:
                    self.state_of_dodge_asteroids, reward_dodge_asteroids, is_done_dodge, _, _ = self.dodge_asteroids.step(
                        action=0)
                    self.state_of_collect_asteroids, reward_collect_asteroids, is_done_collect, _, _ = self.collect_asteroids.step(
                        action=1)
            ...
            case 3:
                ...
                reward_dodge_asteroids = reward_collect_asteroids = -5

                is_done_dodge = is_done_collect = False
        ...
        self.step_counter += 1
        return self.state, reward_dodge_asteroids + reward_collect_asteroids, is_done_dodge or is_done_collect, False, {}
    ...
    def render(self):
        ...
        draw.text(((W - w_collect), (H - h_collect) / 2), ascii_text_collect, fill="black")

        # Save Image
        if not os.path.isdir(self.ROOT_DIR + "src/custom_envs/moonlander" + "/rendering/" + str(
                self.episode_counter)):
            os.mkdir(self.ROOT_DIR + "src/custom_envs/moonlander" + "/rendering/" + str(
                self.episode_counter))
        im.save(
            self.ROOT_DIR + "src/custom_envs/moonlander"
            + "/rendering/"
