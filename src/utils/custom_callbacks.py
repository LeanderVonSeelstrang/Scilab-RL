...
from utils.custom_evaluation import evaluate_policy_hierarchical as custom_evaluate_policy_hierarchical
from utils.custom_evaluation import evaluate_policy_controlloss as custom_evaluate_policy_controlloss
...
class CustomEvalCallbackControlloss(EvalCallback):
    """
    Callback for evaluating an agent with controlloss
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            episode_rewards, episode_lengths = custom_evaluate_policy_controlloss(
                model=self.model,
                env=self.eval_env,
                n_eval_episodes=self.n_eval_episodes,
                render=self.render,
...
class CustomHierarchicalEvalCallback(EvalCallback):
    """
    Callback for evaluating an agent.
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            episode_rewards, episode_lengths, episode_number_of_crashed_or_collected_objects = custom_evaluate_policy_hierarchical(
                self.model,
                self.eval_env,
                n_eval_episodes=self.n_eval_episodes,
                render=self.render,
